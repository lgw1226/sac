...
class Agent():
    ...
    def get_action(self, o, a=None):
        ...
        logstd = torch.clip(mean_logstd[:,act_dim:], self.logstd_min, self.logstd_max)
        dist = torch.distributions.Normal(mean, torch.exp(logstd))

        if not a:
            a = dist.rsample()
        logp = dist.log_prob(a) - torch.log(1 + 1e-5 - torch.square(F.tanh(a))).sum(-1, keepdim=True)
    ...
    def update(self, o1, a, r, o2, d):
        ...
        q1_loss = torch.square(q1 - qt.detach()).mean()
        q2_loss = torch.square(q2 - qt.detach()).mean()
        ...
        p_loss = -(torch.min(*self.get_value(o1, a1)) - alpha.detach() * logp1).mean()
        ...
        return ((q1_loss + q2_loss) / 2).item(), p_loss.item(), alpha_loss.item(), alpha.mean().item()
