...
def train(args):
    ...
    # later will be replaced with argparser arguments
    ...
    ac_lim = float(np.mean(env.action_space.high))

    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    agent = agents.SACAgent(ob_dim, ac_dim, ac_lim, lr=lr, alpha=alpha, device=device)
    ...
    for _ in range(start_step):
        with torch.no_grad():
            ...
            ac = agent.p.get_action(ob_t).cpu().numpy()
    ...
    for _ in range(num_test_episodes):
        ...
        for _ in count():
            ...
            with torch.no_grad():
                ...
                ac = agent.p.get_action(ob_t, eval=True).cpu().numpy().ravel()
